...
public class SameSetOfChar {
    public static void main(String[] args) {
        String words[] = {"ate", "tea", "no", "on", "cat", "act"};
//        SameSetOfChar obj = new SameSetOfChar();
        wordsWithSameChar(words);
    }

    static void wordsWithSameChar(String[] words)
    {
            int n = words.length;
            for (int i = 0; i < n; i++) {
                if (words[i].length() == 0) {
